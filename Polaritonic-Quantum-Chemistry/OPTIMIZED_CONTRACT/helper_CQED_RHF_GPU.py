...
def cqed_rhf(lambda_vector, molecule_string, psi4_options_dict):
    ...
    # form guess density
    D = torch.einsum("pi,qi->pq", Cocc, Cocc)  # [Szabo:1996] Eqn. 3.145, pp. 139
    ...
    mu_exp_x = torch.einsum("pq,pq->", 2 * mu_ao_x, D)
    #mu_exp_x = oe.contract("pq,pq->", 2 * mu_ao_x, D, optimize='optimal')
    mu_exp_y = torch.einsum("pq,pq->", 2 * mu_ao_y, D)
    #mu_exp_y = oe.contract("pq,pq->", 2 * mu_ao_y, D, optimize='optimal')
    mu_exp_z = torch.einsum("pq,pq->", 2 * mu_ao_z, D)
    ...
    Eold = 0.0
    E_1el_crhf = torch.einsum("pq,pq->", H_0 + H_0, D)
    #E_1el_crhf = oe.contract("pq,pq->", H_0 + H_0, D, optimize='optimal')
    E_1el = torch.einsum("pq,pq->", H + H, D)
    ...
    for SCF_ITER in range(1, maxiter + 1):
        ...
        #J = oe.contract("prqs,rs->pq", I, D, optimize="optimal")
        #K = oe.contract("prqs,rs->pq", I, D, optimize="optimal")

        # Pauli-Fierz 2-e dipole-dipole terms, line 2 of Eq. (12) in [McTague:2021:ChemRxiv]
        M = torch.einsum("pq,rs,rs->pq", l_dot_mu_el, l_dot_mu_el, D)
        #M = oe.contract("pq,rs,rs->pq", l_dot_mu_el, l_dot_mu_el, D, optimize='optimal')
        N = torch.einsum("pr,qs,rs->pq", l_dot_mu_el, l_dot_mu_el, D)
...
# diis_e = torch.einsum("ij,jk,kl->il", F, D, S) - torch.einsum("ij,jk,kl->il", S, D, F)
        diis_e = oe.contract("ij,jk,kl->il", F, D, S, optimize='optimal') - torch.einsum("ij,jk,kl->il", S, D, F)
        diis_e = A.dot(diis_e).dot(A)
        dRMS = np.mean(diis_e ** 2) ** 0.5

        # SCF energy and update: [Szabo:1996], Eqn. 3.184, pp. 150
        # Pauli-Fierz terms Eq. 13 of [McTague:2021:ChemRxiv]
# SCF_E = torch.einsum("pq,pq->", F + H, D) + Enuc + d_c
