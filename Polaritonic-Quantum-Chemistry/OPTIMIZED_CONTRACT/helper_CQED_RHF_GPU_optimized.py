...
device = torch.device('mps')  # Use Metal backend for M2 Ultra GPU
torch.set_float32_matmul_precision('high')  # Enable higher precision optimizations
...
def cqed_rhf(lambda_vector, molecule_string, psi4_options_dict):
    ...
    C = torch.as_tensor( np.asarray(wfn.Ca()), dtype=torch.float32, device=device )
    ...
    # form guess density
    # D = oe.contract("pi,qi->pq", Cocc, Cocc)  # [Szabo:1996] Eqn. 3.145, pp. 139
    D = oe.contract("pi,qi->pq", Cocc, Cocc, optimize='optimal')  # [Szabo:1996] Eqn. 3.145, pp. 139

    # Integrals required for CQED-RHF
    # Ordinary integrals first
    V = torch.as_tensor(np.array(mints.ao_potential(), dtype=np.float32), dtype=torch.float32, device=device)
    T = torch.as_tensor(np.asarray(mints.ao_kinetic(),dtype=np.float32), dtype=torch.float32, device=device)
    I = torch.as_tensor(np.asarray(mints.ao_eri(),dtype=np.float32), dtype=torch.float32, device=device)
    ...
    mu_ao_x = torch.as_tensor(np.asarray(mints.ao_dipole())[0], dtype=torch.float32, device=device)
    mu_ao_y = torch.as_tensor(np.asarray(mints.ao_dipole())[1], dtype=torch.float32, device=device)
    mu_ao_z = torch.as_tensor(np.asarray(mints.ao_dipole())[2], dtype=torch.float32, device=device)
    ...
    # mu_exp_x = oe.contract("pq,pq->", 2 * mu_ao_x, D)
    mu_exp_x = oe.contract("pq,pq->", 2 * mu_ao_x, D, optimize='optimal')
    # mu_exp_y = oe.contract("pq,pq->", 2 * mu_ao_y, D)
    mu_exp_y = oe.contract("pq,pq->", 2 * mu_ao_y, D, optimize='optimal')
    # mu_exp_z = oe.contract("pq,pq->", 2 * mu_ao_z, D)
    mu_exp_z = oe.contract("pq,pq->", 2 * mu_ao_z, D, optimize='optimal')
    ...
    # quadrupole arrays
    Q_ao_xx = torch.as_tensor(np.asarray( mints.ao_quadrupole())[0], dtype=torch.float32, device=device)
    Q_ao_xy = torch.as_tensor(np.asarray( mints.ao_quadrupole())[1], dtype=torch.float32, device=device)
    Q_ao_xz = torch.as_tensor(np.asarray( mints.ao_quadrupole())[2], dtype=torch.float32, device=device)
    Q_ao_yy = torch.as_tensor(np.asarray( mints.ao_quadrupole())[3], dtype=torch.float32, device=device)
    Q_ao_yz = torch.as_tensor(np.asarray( mints.ao_quadrupole())[4], dtype=torch.float32, device=device)
    Q_ao_zz = torch.as_tensor(np.asarray( mints.ao_quadrupole())[5], dtype=torch.float32, device=device)
    ...
    Eold = 0.0
    # E_1el_crhf = oe.contract("pq,pq->", H_0 + H_0, D)
    E_1el_crhf = oe.contract("pq,pq->", H_0 + H_0, D, optimize='optimal')
    # E_1el = oe.contract("pq,pq->", H + H, D)
    E_1el = oe.contract("pq,pq->", H + H, D, optimize='optimal')
    ...
    for SCF_ITER in range(1, maxiter + 1):
        ...
        # #M = oe.contract("pq,rs,rs->pq", l_dot_mu_el, l_dot_mu_el, D)
        #M = oe.contract("pq,rs,rs->pq", l_dot_mu_el, l_dot_mu_el, D, optimize='optimal')
        # #N = oe.contract("pr,qs,rs->pq", l_dot_mu_el, l_dot_mu_el, D)
        ...
        #print(f"Einsum Execution Time: {end_time - start_time:.5f} seconds")
        ...
        # diis_e = oe.contract("ij,jk,kl->il", F, D, S) - oe.contract("ij,jk,kl->il", S, D, F)
        diis_e = oe.contract("ij,jk,kl->il", F, D, S, optimize='optimal') - oe.contract("ij,jk,kl->il", S, D, F)
        diis_e = A.dot(diis_e).dot(A)
        dRMS = np.mean(diis_e ** 2) ** 0.5

        # SCF energy and update: [Szabo:1996], Eqn. 3.184, pp. 150
        # Pauli-Fierz terms Eq. 13 of [McTague:2021:ChemRxiv]
        # SCF_E = oe.contract("pq,pq->", F + H, D) + Enuc + d_c
        SCF_E = oe.contract("pq,pq->", F + H, D, optimize='optimal') + Enuc + d_c
    ...
    #   SCF_E_One = oe.contract("pq,pq->", rhf_one_e_cont, D)
    SCF_E_One = oe.contract("pq,pq->", rhf_one_e_cont, D, optimize='optimal')
    # SCF_E_Two = oe.contract("pq,pq->", rhf_two_e_cont, D)
    SCF_E_Two = oe.contract("pq,pq->", rhf_two_e_cont, D, optimize='optimal')
    # CQED_SCF_E_Two = oe.contract("pq,pq->", pf_two_e_cont, D)
    CQED_SCF_E_Two = oe.contract("pq,pq->", pf_two_e_cont, D, optimize='optimal')

    # CQED_SCF_E_D_PF = oe.contract("pq,pq->", 2 * d_PF, D)
    CQED_SCF_E_D_PF = oe.contract("pq,pq->", 2 * d_PF, D, optimize='optimal')
    # CQED_SCF_E_Q_PF = oe.contract("pq,pq->", 2 * Q_PF, D)
    CQED_SCF_E_Q_PF = oe.contract("pq,pq->", 2 * Q_PF, D, optimize='optimal')
